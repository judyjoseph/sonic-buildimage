...
{
    SYSTEM_WARM_START=`sonic-db-cli -n "$NET_NS" STATE_DB hget "WARM_RESTART_ENABLE_TABLE|system" enable`
    SERVICE_WARM_START=`sonic-db-cli -n "$NET_NS" STATE_DB hget "WARM_RESTART_ENABLE_TABLE|${SERVICE}" enable`
...
function validate_restore_count()
{
    if [[ x"$WARM_BOOT" == x"true" ]]; then
        RESTORE_COUNT=`sonic-db-cli -n "$NET_NS" STATE_DB hget "WARM_RESTART_TABLE|orchagent" restore_count`
...
function wait_for_database_service()
{
    ...
    until [[ $(sonic-db-cli -n "$NET_NS" CONFIG_DB GET "CONFIG_DB_INITIALIZED") ]];
        do sleep 1;
    done
}
...
start() {
    ...
    if [[ x"$WARM_BOOT" != x"true" ]]; then
        ...
        sonic-db-cli -n "$NET_NS" APPL_DB FLUSHDB
        sonic-db-cli -n "$NET_NS" ASIC_DB FLUSHDB
        sonic-db-cli -n "$NET_NS" COUNTERS_DB FLUSHDB
        sonic-db-cli -n "$NET_NS" FLEX_COUNTER_DB FLUSHDB
